<script type="text/javascript">
    function onClickSelesai() {
        ...
        if (materi === '' || nim_mhs === '' || pin_mhs === '') {
            ...
            var url = "<?php echo site_url('absensi_dosen/kelas_selesai') ?>"
            + "/" +  kode + "/" +  materi + "/" + pin_mhs + "/" + nim_mhs;
            var xmlhttp = new XMLHttpRequest();
            xmlhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    var verif = this.responseText;
                    if(verif == '00'){
                        var display = "<div class='card-body'><h3 class='text-center'>Kelas Telah Selesai</h3>"
